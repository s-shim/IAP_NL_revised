...
def doubleLS(doubleQ1,doubleQ2,profiling,logSum,iteration,machineName,networkID,rep,tic):
    ...
    result.to_csv(r'%s/3_result_LS/process/process_LS_logSum%s_Network%s_Rep%s_doubleQ1%s_doubleQ2%s.csv'%(networkID,int(logSum*100),networkID,rep,doubleQ1,doubleQ2), index = False)#Check
...
for (networkID,repNum) in [('yt',1)]:
    ...
    for rep in range(repNum):            
        ...
        print('rep =',rep)
        nodes = pd.read_csv('%s/nodes_%sproducts_choice_revised/nodes_%s_%s.csv'%(networkID,numProducts,networkID,rep))
        ...
        if __name__ == '__main__':
            ...
            loptSolution.to_csv(r'%s/3_result_LS/lopt/lopt_DLS_Net%s_Rep%s_logSum%s.csv'%(networkID,networkID,rep,int(logSum*100)), index = False)#Check        
            ...
            grandTable.to_csv(r'%s/3_result_LS/summary/Summary_LS_Double_%s_logSum%s_%s.csv'%(networkID,networkID,int(logSum*100),machineName), index = False)#Check
