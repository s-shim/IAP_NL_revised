...
def doubleLS(doubleQ1,doubleQ2,profiling,logSum,iteration,machineName,networkID,rep,tic):
    ...
    result.to_csv(r'2_result_Sensitivity/3_result_LS/process/process_LS_logSum%s_Network%s_Rep%s_doubleQ1%s_doubleQ2%s.csv'%(int(logSum*100),networkID,rep,doubleQ1,doubleQ2), index = False)#Check
...
def doubleOption_revised(doubleQ1,doubleQ2,profiling,logSum):
    ...
    for u in nodeList:
        ...
        is_offered[u,doubleQ1] = 1
        is_offered[u,doubleQ2] = 0
        optionsOffered[u] = [doubleQ1]
        tpw[u] = pow(pw[u,doubleQ1],1/logSum)                    
        prob1 = pw[u,doubleQ1] / (1 + pw[u,doubleQ1])
        revenue[u] = price[doubleQ1] * prob1

        prob2 = pw[u,doubleQ2] / (1 + pw[u,doubleQ2])
        rev_2 = price[doubleQ2] * prob2
        if revenue[u] < rev_2:
            revenue[u] = rev_2
            is_offered[u,doubleQ1] = 0
            is_offered[u,doubleQ2] = 1
            optionsOffered[u] = copy.deepcopy([doubleQ2])
            tpw[u] = pow(pw[u,doubleQ2],1/logSum)                    
        ...
        rev_3 = price[doubleQ1] * prob3_1 + price[doubleQ2] * prob3_2
        if revenue[u] < rev_3:
            revenue[u] = rev_3
            is_offered[u,doubleQ1] = 1
            is_offered[u,doubleQ2] = 1
            optionsOffered[u] = copy.deepcopy([doubleQ1,doubleQ2])
            tpw[u] = tpw3                    

        totalRevenue += revenue[u]
        optionsCount += len(optionsOffered[u])
...
#doubleOptionList = [(1,2),(1,3),(2,3),(2,4),(3,4),(4,5),(4,6),(5,6)]


## Instances
for (networkID,repNum) in [(9,50),(5,50),(3,50),(4,50),(2,50)]:#[(9,50),(5,50),(3,50),(4,50),(2,50),(0,10),(8,10),(7,10),(6,10),(1,10)]:
    ...
    for rep in range(repNum):            
        ...
        if __name__ == '__main__':
            ...
            loptSolution.to_csv(r'2_result_Sensitivity/3_result_LS/lopt/lopt_DLS_Net%s_Rep%s_logSum%s.csv'%(networkID,rep,int(logSum*100)), index = False)#Check        
            ...
            grandTable.to_csv(r'2_result_Sensitivity/3_result_LS/summary/Summary_LS_Double_%s_logSum%s_%s.csv'%(networkID,int(logSum*100),machineName), index = False)#Check
